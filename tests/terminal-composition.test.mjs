...
test('terminal top-level keys prioritize operator scan order', async () => {
  ...
  const activeSettingsProfileIdIndex = keys.indexOf('active_settings_profile_id');
  ...
  assert.ok(marketRadarIndex > forecastSummaryIndex);
  ...
  assert.ok(watchlistSummaryIndex > migrationIndex);
  assert.ok(watchlistIndex > watchlistSummaryIndex);
  ...
  assert.ok(activeSettingsProfileIdIndex > settingsSummaryIndex);
