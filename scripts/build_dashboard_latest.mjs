...
function confidenceFromSignals({ componentScores }) {
  ...
  const spread = Math.max(...available) - Math.min(...available);
...
// ---- Config Readers (State + MSA) ----
function readConfigSafe(path, arrayKey, maxKey, defaultMax = 0) {
  try {
    if (!fs.existsSync(path)) return { max: defaultMax, items: [] };
    const raw = fs.readFileSync(path, "utf8");
    const parsed = JSON.parse(raw);

    const items = Array.isArray(parsed[arrayKey]) ? parsed[arrayKey] : [];
    const max = typeof parsed[maxKey] === "number" ? parsed[maxKey] : defaultMax;
    ...
    return { max, items: usable };
  } catch {
    return { max: defaultMax, items: [] };
  }
}

async function fetchConfiguredPermitSignals({ configPath, arrayKey, maxKey, labelPrefix }) {
  const cfg = readConfigSafe(configPath, arrayKey, maxKey, 0);
  const limit = Math.max(0, Math.min(cfg.max || 0, 50));
  const entries = (cfg.items || []).slice(0, limit);

  if (!entries.length) return { signals: [], loaded: 0, expected: 0 };
    ...
    entries.map(async (s) => {
      try {
        ...
        return makeSignal({
          ...
          region: `${labelPrefix}${s.name || s.code || ""}`.trim(),
          units: "units",
          history: hist
        });
      } catch {
...
function buildDashboard({
  ...
  const sLoaded = stateSignalsInfo?.loaded ?? 0;
  const sExpected = stateSignalsInfo?.expected ?? 0;
  const mLoaded = msaSignalsInfo?.loaded ?? 0;
  const mExpected = msaSignalsInfo?.expected ?? 0;

  const stateNote = sExpected > 0 ? ` • States: ${sLoaded}/${sExpected}` : "";
  const msaNote = mExpected > 0 ? ` • MSAs: ${mLoaded}/${mExpected}` : "";
...
// ---- Main ----
(async () => {
  const stateSignalsInfo = await fetchConfiguredPermitSignals({
  ...
  const msaSignalsInfo = await fetchConfiguredPermitSignals({
